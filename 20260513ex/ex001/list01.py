...
# fruits = ['사과','포도','바나나']    # 컨테이너 자료형 : list(리스트)
# print(f'fruits: {fruits}')
# print(f'fruits type: {type(fruits)}')

# 컨테이너 자료형은 복수형으로 지정할 것 (다수의 자료를 묶어놨으니)
# 파이썬에서 컨테이너 자료형으로는 리스트(list), 튜플(tuple), 딕셔너리(dictionary) 가 있다에요.
...
# pop() vs del
# nums = [1,2,3,4,5,6]
# deletedNums = nums.pop(3)   # [1, 2, 3, 5, 6]
# print(f'deletedNums: {deletedNums}')
...
print(f'평균: {average:.2f}')       #지향하자   # 소수점 두자릿수로 하려면[.2f] 입력
# print(f'평균: {total / 6}')               #지양하자

# 합격 여부
if undersubject > 0 or average < 60:
    print(f'아쉽숩니다. 다음 기회에...')
else:
    print(f'축! 합격!!')