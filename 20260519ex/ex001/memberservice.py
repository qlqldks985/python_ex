...
while flag:
    ...
    if userSelectedMenuNum == SIGN_UP:            # 회원가입
        uId = input('Input member ID: ')
        if not isMember(uId):        # False: 회원이 없는 경우(회원가입 진행O)   True: 회원이 있는 경우(회원가입 진행X)
        ...
        while True:
            ...
            print(uId in members)

            if uId in members:   # gildong O or X
                uInfo = members[uId]
                if uInfo['uPw'] == uPw:
    ...
    elif userSelectedMenuNum == PRINT_MY_INFO:    # 나의 정보 출력
        uId = input('Input member ID: ')
        uPw = input('Input member PW: ')
    ...
    elif userSelectedMenuNum == PRINT_ALL_MEMBER_INFO:   # 모든 회원 정보 출력
        for key, value in members.items():
            print(f'{key}님의 정보----------------')
            printAllMemberInfo(value)
            print('-' * 30)

    elif userSelectedMenuNum == SYSTEM_SHUTDOWN:  # 종료
        flag = False
        print('Good bye')