...
while flag:
    ...
    if session.signInedMemberId == '':
        # sign out 상태
        menuNum = int(input(f'1. sign-up    2. sign-in   6. write    7. read    99.end'))
    else:
        # sign in 상태
        menuNum = int(input(f'3. modify     4.delete      5. sign-out     6. write    7. read     99.end'))
    ...
    if menuNum == config.SIGN_UP:
        ...
        print('3.modify')
        '''
        id, pw, email, phone 이중에서 어떤 정보들만 수정가능케 할 것인지 정해야 합니다.
        ...
        즉 session.signInedMemberId 에서 현재 로그인 되어 있는 회원 ID를 가져와서
        사용하면 됩니다.
        '''
        ...
        print('4.delete')
        '''
        - 현재 로그인 되어 있는 회원의 ID를 session.signInedMemberId 에서 가져와서
        - 해당하는 회원의 정보를 member_db.memberDB 에서 삭제 합니다.
        '''
        currentSignInedMemberID = session.signInedMemberId
        ...
        - 메뉴를 변경해야겠다.
        로그인 값을 없애야겠다.
        session 모듈에 signInedMemberId 변수에 있는거죠? spq!
        '''
        ...
        if session.signInedMemberId == '':
            ...
            while True:
                diaryTxt = input('10 글자 이하의 짧은 일기를 작성하세요.')
                if len(diaryTxt) > 10:
        ...
        if session.signInedMemberId == '':
            ...
            print(f'deepCopyDiaries type: {type(deepCopyedDiaries)}')
            print(f'deepCopyDiaries: {deepCopyedDiaries}')
