# 딕셔너리(dictionary)
# 리스트와 함께 파이썬 프로그램에서 많이 사용하는 컨테이너 자료형입니다. 리스트가 인덱스를 이용하여 아이템을 참조한다면,
# 딕셔너리는 인덱스 대신 키(key)를 이용하여 아이템을 참조합니다.
# 딕셔너리는 키(key) 와 값(value)의 쌍으로 이루어져 있습니다. 이는 마치 개인 사물함의 구조와 같습니다 # " 내 키는 3번 사물함만 열 수 있어! "

# 딕셔너리 정의
# 딕셔너리는 {}

# ages = {'박찬호': 48, '박지성': 40, '박세리': 50, "이승엽": 43, '박찌성':40} # 키값은 중복되면 안된다. 오직 하나만 있어야 한다. 벨류값은 같아도 상관없다.
# print(f'ages: {ages}')
...
# print(f'dicts: {dicts}')                # {'name': '박찬호', 'age': 20, 'addr': '대전 중구', 'hobby': ['축구', '농구', '배구']}
...
#     '몸무게':87.5
# }
# print(f'dicNontainer: {dicNontainer}')

# dicNontainer['연락처'] = '010-1234-5678'
# print(f'dicNontainer: {dicNontainer}')

# 조회(Read)
# print(f'이름: {dicNontainer['이름']}')
...
# 삭제(Delete)
# del dicNontainer['몸무게']
# print(f'dicNontainer: {dicNontainer}')

# 부가 가능들
# 아이템 개수 조회
# print(f'아이템 개수: {len(dicNontainer)}')

# 전체키 & 벨류를 조회
# 전체키
# dicKeys = dicNontainer.keys()       #(['이름', '나이', '주소', '취미', '연락처']) # 키값을 더한다(붙힌다)
# print(f'dickeys: {dicKeys}')

# for key in dicKeys:
#     print(f'{key}: {dicNontainer[key]}')        # 키값도 함께 조회
    
# 전체 벨류
# dicValues = dicNontainer.values()     # (['홍길동', 25, '대전 중구', ['축구', '수영', '조깅'], '010-1234-5678'])
# print(f'dicValues: {dicValues}')

# 키와 벨류를 한방에 조회
# for key,value in dicNontainer.items():      # 이름: 홍길동 나이: 25 주소: 대전 중구 취미: ['축구', '수영', '조깅'] 연락처: 010-1234-5678
#     print(f'{key}: {value}')
# print(dicNontainer.items())
# print(type(dicNontainer.items()))       # <class 'dict_items'>
# 중간고사 성적 관리 프로그램 만들기
'''
...
scores['파이썬'] = 'A'
scores['OS'] = 'A+'
print(f'파이썬: {scores['파이썬']}')        # A
print(f'OS: {scores['OS']}')                # A+
# 4
# Java'와 '시스템'의 성적을 각각 'F'와 'A'로 수정한다.
scores['Java'] = 'F'
scores['시스템'] = 'A'
print(f'scores: {scores}')
# 5
...
for key in scores.keys():
    totalScore += creditScore[scores[key]]
