# 자주 사용하는 모듈: 내장 모듈
# 이러한 내장 모듈들은 절대로 외우는 것이 아니다.
# 따라서 가볍게 훑어보고 필요할 때 찾아서 사용하면 된다.
...
sample(range(n1, n2, n3)): n1 이상 n2 미만의 난수 n3개 발생(중복 없이)
...
print(f'random(): {random.random()}')   # 0 이상 1 미만 난수
print(f'randint(): {random.randint(0, 9)}')   # 0 이상 9 이하 난수
print(f'randrange(): {random.randrange(0, 9)}')   # 0 이상 9 미만 난수
print(f'sample(): {random.sample(range(0, 10), 3)}')   # 0 이상 9 미만 난수
